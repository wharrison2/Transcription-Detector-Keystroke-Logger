...
class TestSuite():
    ...
    def get_avg_performance_neural(self, features=ALL_FEATURES, folds=10, epochs=300, p_train=0.75, shape=[30, 10], reg_param=0.01, reg_type='l2', show_plot=True, show_train=True):
    ...
    def get_avg_performance_xg(self, features=ALL_FEATURES, ests=5, lr=0.3, d=3, gamma=0.01, folds=500, p_train=0.75, show_plot=True, show_train=True):
        feature_data = extract_features(self.tasks, features)
        feature_data_normalized = normalize_features(feature_data)
        train_accuracies = []
        accuracies = []
        ...
        for i in range(folds):
            ...
            bst = XGBClassifier(n_estimators=ests, max_depth=d, learning_rate=lr, gamma=gamma, objective='binary:logistic')
    ...
    def get_avg_confusion_xg(self, features=ALL_FEATURES, ests=5, lr=0.3, d=3, folds=500, p_train=0.75):
        ...
        for _ in range(folds):
            train_set, test_set, train_labels, test_labels = split_data(feature_data_normalized, self.labels, p_train=p_train)
            bst = XGBClassifier(n_estimators=ests, max_depth=d, learning_rate=lr, objective='binary:logistic')
            bst.fit(train_set, train_labels)
            preds = bst.predict(test_set)
            tp += np.sum((preds == 1) & (test_labels == 1))
            tn += np.sum((preds == 0) & (test_labels == 0))
